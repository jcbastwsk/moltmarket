...
router.post('/', (req, res) => {
  const { name, lnAddress } = req.body;
  if (!name || !lnAddress) return res.status(400).json({ error: 'name and lnAddress required' });
  if (name.length > 100) return res.status(400).json({ error: 'name too long' });

  const clean = sanitize(req.body, ['name', 'description']);
  const db = getDb();

  const existing = db.prepare("SELECT id FROM agents WHERE lnAddress = ?").get(lnAddress);
  ...
  db.prepare(`INSERT INTO agents (id, name, description, skills, lnAddress, apiKey, modelProvider, createdAt) VALUES (?, ?, ?, ?, ?, ?, ?, ?)`).run(
    id, clean.name, clean.description || '', JSON.stringify(skills), lnAddress, apiKey, req.body.modelProvider || 'openrouter', now
  );

  res.status(201).json({ id, apiKey, name: clean.name, lnAddress });
});
...
router.get('/:id', (req, res) => {
  const db = getDb();
  const agent = db.prepare(`SELECT id, name, description, skills, lnAddress, reputation, tasksCompleted, tasksAccepted, totalEarned, modelProvider, createdAt FROM agents WHERE id = ?`).get(req.params.id) as any;
  if (!agent) return res.status(404).json({ error: 'agent not found' });
  agent.skills = JSON.parse(agent.skills || '[]');

  const recentTasks = db.prepare(`SELECT t.id, t.title, t.category, t.bountySats, t.status, d.rating FROM tasks t LEFT JOIN deliverables d ON d.taskId = t.id AND d.agentId = ? WHERE t.assignedAgentId = ? ORDER BY t.updatedAt DESC LIMIT 10`).all(req.params.id, req.params.id);
